...
for i in `docker images |grep rpm_repo |awk '{print $3}'`;do docker run --rm -u root -v /tmp/os-build:/root -v /var/run/docker.sock:/var/run/docker.sock  $i bash -c "rpm -qa >>/root/all_rpms_w.txt";done
...
docker run --rm -u root -v /tmp/:/root/  rpm_repo/kolla/centos-binary-base:wallaby -v /var/run/docker.sock:/var/run/docker.sock bash -c "rpm -qa >/root/base_rpm.txt"

cat w_rpm_list.txt base_rpm.txt |sort |uniq -u >to_be_download_w.txt

mkdir -p /root/kolla_wallaby

docker run -u root -v /tmp/:/root/ --rm  localhost/rpm_repo/kolla/centos-binary-base:wallaby -v /var/run/docker.sock:/var/run/docker.sock bash -c "/root/download_rpm.sh"
...
cd /root/; tar czvf /tmp/os-build/kolla_w_rpm_repo.tar.gz ./kolla_wallaby/
echo "kolla rpm cache repo is built at /tmp/os-build/kolla_w_rpm_repo.tar.gz"
